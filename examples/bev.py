...
def main():
    ...
    p_drive = sc.params.P_BEV_drive_KGB.where(
        sc.params.P_BEV_drive_KGB.index.get_level_values(0) % 24 < 12, 20
    )
    y_avail = (p_drive == 0).astype(int)
    sc.update_params(P_BEV_drive_KGB=p_drive, y_BEV_avail_KGB=y_avail)
