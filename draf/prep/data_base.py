...
class DataBase:
    ...
    k_BES_maint_ = ParDat(name="k_BES_maint_", data=0.02, doc=Descs.RMI.en, src=SRC.Juelch_2016)
    k_BES_outPerCapa_ = ParDat(name="k_BES_outPerCapa_", data=0.7, doc=f"Ratio discharging power / capacity", src=SRC.Figgener_2021)
    k_CHP_maint_ = ParDat(name="k_CHP_maint_", data=0.06 + 0.02, doc=Descs.RMI.en, src=SRC.VDI2067)
    k_CM_maint_ = ParDat(name="k_CM_maint_", data=0.01 + 0.015, doc=Descs.RMI.en, src=SRC.VDI2067)
    k_HOB_maint_ = ParDat(name="k_HOB_maint_", data=0.04, doc=Descs.RMI.en)  # TODO
    k_HP_maint_ = ParDat(name="k_HP_maint_", data=0.01 + 0.015, doc=Descs.RMI.en, src=SRC.VDI2067)
    k_PV_maint_ = ParDat(name="k_PV_maint_", data=0.02, doc=Descs.RMI.en, src=SRC.ISE_2018)
    k_TES_maint_ = ParDat(name="k_TES_maint_", data=0.001, doc=Descs.RMI.en, src=SRC.FFE_2016)
